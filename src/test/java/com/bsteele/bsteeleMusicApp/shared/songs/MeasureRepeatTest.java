...
public class MeasureRepeatTest {
    ...
    public void parseMarkup() {
        ...
        measureRepeat = MeasureRepeat.parse("[A B C D [ x2 E F", 4);
        assertNull(measureRepeat);

        String s = "[A B C D ] x2 ";
        StringBuffer sb = new StringBuffer(s);
        MeasureRepeat refRepeat = MeasureRepeat.parse(sb, 4);
        assertNotNull(refRepeat);
        assertEquals(s, refRepeat.toMarkup());
        assertEquals(0, sb.length());

        s = "[A B C D ] x2 E F";
        measureRepeat = MeasureRepeat.parse(s, 4);
        ...
        //  test with comment
        refRepeat = MeasureRepeat.parse("   A B(yo)C D  x2 E F", 4);
        assertNotNull(refRepeat);
